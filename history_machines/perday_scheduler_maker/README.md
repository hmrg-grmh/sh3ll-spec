...
但，正所谓「有再二就会有再三」。然而， *方式 3* 也并不是结束。

这是对「历史机器」库的一个使用例。当然，在这里，这个使用例本身就揭示了这个库的诞生过程。

而本使用例（本工具），核心就是这个把模板转换为目标结果，这个核心的关键就是上面扯了很多的 *方式 n* ，而它们最终会得出的，就是这个：
...
这个每日调度工具生成器，既是对[库代码](../src.sh) `historisch` 如何使用的说明，也是对它作为一个结论的得出过程的阐释。
