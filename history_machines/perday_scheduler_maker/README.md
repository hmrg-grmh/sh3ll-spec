...
然后看看它印出来的两行代码就是 *方式 2* 里的结果中共性很强的部分。

代码变成字符流，丢给 `eval` ，就等于代码本身。最后再加上一个 `cat -` 结尾就能执行了。
...
仔细看看，它完全只有做一样事情：通过上面提到的使用方式使用 `History` 这个抽象（共性）。

接下来，补上 `History` ，再加一些使用说明。完整的就是这样了：
